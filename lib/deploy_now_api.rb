...
class DeployNowApi
  ...
  def update_deployment_status
    @client["/v3/accounts/me/project/#{@project_id}/branches/#{@branch_id}/deployed"].post(nil,
                                                                                           content_type: 'application/json')
  end

  def get_branch_info
    response = @client["/v3/accounts/me/project/#{@project_id}"].get
    project = JSON.parse(response.body)
    is_production_branch = project['productionBranch']['id'] == @branch_id
    branch = @client["/v3/accounts/me/project/#{@project_id}/branches/#{@branch_id}"].get
    {
  ...
  def configure_cron_jobs(jobs)
    @client["/v3/accounts/me/project/#{@project_id}/branches/#{@branch_id}/cron-jobs"].put(jobs.to_json,
                                                                                           content_type: 'application/json')
  end
end
