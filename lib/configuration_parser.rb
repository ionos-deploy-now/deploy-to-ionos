  ...
  class << self
    def parse(options)
      ...
      begin
        ...
        abort "::error file=.deploy-now/config.yaml::unable to parse the .deploy-now/config.yaml config file\n#{e}"
      end

      abort '::error file=.deploy-now/config.yaml::version must be specified in .deploy-now/config.yaml config file' unless config.include? 'version'

      version = config['version'].to_s
      abort "::error file=.deploy-now/config.yaml::unknown version: #{version}" unless version == '1.0'
    ...
    def parse_1_0(config, deployment_folder, bootstrap)
      ...
      if config.include? 'deploy'
        abort "::error file=.deploy-now/config.yaml::only 'bootstrap' or 'recurring' are allowed for deploy.force in .deploy-now/config.yaml" unless config['deploy']['force'].eql?('bootstrap') || config['deploy']['force'].eql?('recurring') || config['deploy']['force'].nil?
    ...
    def validate_cron_jobs(jobs)
      abort '::error file=.deploy-now/config.yaml::cron-jobs must be a list' unless jobs.is_a? Array
      jobs.each do |job|
        abort "::error file=.deploy-now/config.yaml::A cron job requires the fields 'command' and 'schedule' in .deploy-now/config.yaml" unless job.include?('command') && job.include?('schedule')
      end
    end
  end
end
